...
let memoryRoot = path.join(__dirname, 'memory');

function setMemoryRoot(newRoot) {
  if (typeof newRoot === 'string' && newRoot.trim().length > 0) {
    memoryRoot = newRoot;
  }
}
...
function loadQuickMemory(characterId) {
  const folder = getCharacterFolder(characterId);
  const filePath = path.join(folder, 'quick_memory.json');
  if (!fs.existsSync(filePath)) {
    return null;
  }
  try {
    ...
    console.error('[memoryManager] Failed to read quick memory file:', err.message);
    return null;
  }
}
